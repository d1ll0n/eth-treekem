...
async function groupDump(label, group) {
  ...
  for (let n in group.nodes) {
    ...
    console.log("  ", n, ":", await fingerprint(group.nodes[n].public));
  }
}
...
async function testUserAdd(State) {
  ...
  let creator = await State.oneMemberGroup(EC.randomBytes(32).toString());
  let members = [creator];

  for (let i = 1; i < testGroupSize; ++i) {
    let leaf = EC.randomBytes(32).toString()
...
async function testGroupAdd(State) {
  ...
  for (let i = 1; i < testGroupSize; ++i) {
    let initLeaf = EC.randomBytes(32).toString()
