...
class TreeKEM {
  ...
  async equal(other) {
    ...
    for (let i in this.nodes) {
      ...
      let fp1 = fingerprint(this.nodes[i].public);
      let fp2 = fingerprint(other.nodes[i].public);
      if (fp1 !== fp2) {
        console.log(`not equal: ${i} ${fp1} ${fp2}`)
        return false;
      }
    }
  ...
  static async hashUp(index, size, h) {
    ...
    let secret = nodes[root].secret;
    let hue = Array.from(new Uint8Array(Buffer.from(secret))).reduce((x, y) => x ^ y);
  ...
  async dump(label) {
    ...
    for (let i in this.nodes) {
      console.log("  ", i, ":", fingerprint(this.nodes[i].public));
    }
  }
}
...
async function testEncryptDecrypt() {
  ...
  const seed = hash(new Uint8Array([0,1,2,3]))
